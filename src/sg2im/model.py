...
import src.sg2im.box_utils as box_utils
from src.sg2im.graph import GraphTripleConv, GraphTripleConvNet
from src.sg2im.crn import RefinementNetwork
from src.sg2im.layout import boxes_to_layout, masks_to_layout
from src.sg2im.layers import build_mlp
...
class Sg2ImModel(nn.Module):
  def __init__(self, vocab, image_size=(64, 64), embedding_dim=64,
    ...
    print("Object:", vocab['object_idx_to_name'])
    print("Relation:", vocab['pred_idx_to_name'])
    ...
    print("Object shape:",self.obj_embeddings.weight.shape)
    print("Pred shape:",self.pred_embeddings.weight.shape)

    print("Vector ngẫu nhiên cho mỗi đối tượng có trong vocab:")
    print(self.obj_embeddings.weight)
    print("Object shape:",self.obj_embeddings.weight.shape)
  ...
  def forward(self, objs, triples, obj_to_img=None,
    ...
    print("Vector đặc trưng cho mỗi đối tượng trong Scene Graph được chuyển đổi bởi Graph Convolution Network:")
    print(obj_vecs)
    print("Object shape:", obj_vecs.shape)

    print("Object vectors:", obj_vecs.shape)
    print("Predicate vectors:", pred_vecs.shape)
    ...
    print("Scene Layout:")
    print("Shape layout:", layout.shape)
    print(layout)

    print("\n")

    print("Bouding box của các đối tượng:")
    print("Shape bouding box:", boxes_pred.shape)
    print(boxes_pred)

    print("\n")

    print("Mask của các đối tượng:")
    print("Shape mask:", masks_pred.shape)
    print(masks_pred)

    print("\n")

    print("Hình ảnh được tổng hợp bằng Cascaded Refinement Network dựa trên Scene Layout:")
